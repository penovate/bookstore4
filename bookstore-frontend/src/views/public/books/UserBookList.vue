...
const loadData = async () => {
    ...
    try {
        ...
        if (route.query.genreId) {
            ...
            const targetGenre = genresRes.data.find(g => String(g.genreId) === String(route.query.genreId));
            if (targetGenre) {
...
const filteredBooks = computed(() => {
    ...
    if (selectedGenreId.value !== 'all') {
        result = result.filter(book =>
            book.genres && book.genres.some(g => g.genreId === selectedGenreId.value)
        );
    }
...
// 監聽路由參數變化 (支援同一頁面切換分類)
watch(() => route.query.genreId, (newGenreId) => {
            ...
            <v-card class="rounded-lg elevation-2 border-opacity-75" color="white">
                <v-list class="py-2" density="compact">
                    ...
                    <div class="genre-list-container">
                        <v-list-item v-for="genre in genres" :key="genre.genreId" :value="genre.genreId"
                            active-color="primary" :active="selectedGenreId === genre.genreId"
                            @click="selectGenre(genre.genreId)" rounded="lg" class="mb-1 mx-2 genre-item">
                            <template v-slot:prepend>
                                <v-icon :icon="selectedGenreId === genre.genreId ? 'mdi-leaf' : 'mdi-leaf-off'"
                                    size="small"
                ...
                <v-col cols="12" md="10" lg="10">
                    ...
                    <v-row class="mb-6" align="center">
                        ...
                        <v-col cols="12" sm="7" class="d-flex align-center justify-end flex-wrap gap-4">
                            <!-- 搜尋 -->
                            <v-text-field v-model="search" label="搜尋書籍或作者..." prepend-inner-icon="mdi-magnify"
                                variant="outlined" density="compact" hide-details bg-color="white" class="flex-grow-1"
                                style="max-width: 300px; min-width: 200px;"></v-text-field>

                            <!-- 顯示切換 -->
                            <v-btn-toggle v-model="viewMode" mandatory color="primary" variant="outlined"
                                density="compact" class="ms-2">
                            ...
                            <v-menu v-if="$vuetify.display.smAndDown">
                                <template v-slot:activator="{ props }">
                                    <v-btn v-bind="props" variant="tonal" color="primary"
                                        prepend-icon="mdi-filter-variant" class="d-md-none ms-2">
                                        分類
                                    </v-btn>
                                </template>
                                <v-list>
                    ...
                    <div v-else>
                        ...
                        <v-row v-if="viewMode === 'grid' && filteredBooks.length > 0">
                            <v-col v-for="book in filteredBooks" :key="book.bookId" cols="12" sm="6" md="4" lg="4"
                                xl="3">
                                <BookCard :book="book" />
                            </v-col>
                        </v-row>
                        ...
                        <!-- LIST VIEW -->
                        <div v-else-if="viewMode === 'list' && filteredBooks.length > 0"
                            ...
                            <v-card v-for="book in filteredBooks" :key="book.bookId"
                                ...
                                <div class="flex-grow-1">
                                    ...
                                    <div class="mt-1">
                                        <v-chip v-for="g in book.genres" :key="g.genreId" size="x-small"
                                            color="secondary" variant="flat" class="me-1">
                                            {{ g.genreName }}
                                        </v-chip>
                                    </div>
                                </div>
